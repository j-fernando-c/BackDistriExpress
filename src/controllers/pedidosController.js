...
const validatePedidos = (data) => pedidosSchema.validate(data, { abortEarly: false });
...
const create = async (req, res, next) => {
  try {
    ...
    const { producto_id, estado, fecha, cantidad_productos, total_pedido } = value;
...
const toggleEstado = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message: `Pedido ${result.rows[0].estado === 'Activo' ? 'activado' : 'desactivado'} exitosamente`,
      data: result.rows[0]
    });
  } catch (error) {
    next(error);
